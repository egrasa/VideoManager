    ...
    def generate_single_timeline_frame(
        ...
        try:
            cmd = [
                ...
                '-ss', str(timestamp),
                '-vframes', '1',
                '-vf', 'scale=120:67',  # Timeline frame size (16:9 aspect)
                '-q:v', '5',  # Medium quality for timeline
                '-y',
                frame_path
            ]
            ...
            if result.returncode == 0 and Path(frame_path).exists():
                ...
                logger.warning("Failed to extract frame %d from %s", frame_index, video_path)
                return None
                
        except subprocess.TimeoutExpired:
            logger.error("Timeout extracting frame %d from %s", frame_index, video_path)
            return None
        except (OSError, ValueError) as e:
    ...
    def generate_timeline_frames(
        ...
        for i in range(num_frames):
            ...
            try:
                ...
                if result.returncode == 0 and Path(frame_path).exists():
                    logger.debug("Generated timeline frame %d: %s", i, frame_path)
                    frame_paths.append(frame_path)
                else:
                    logger.warning("Failed to extract frame %d from %s", i, video_path)
                    
            except subprocess.TimeoutExpired:
                logger.error("Timeout extracting frame %d from %s", i, video_path)
            except (OSError, ValueError) as e:
                logger.error("Error extracting frame %d: %s", i, e)
